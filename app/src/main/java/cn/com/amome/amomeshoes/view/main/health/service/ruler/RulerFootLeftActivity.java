 ...
 * @author css
 */
public class RulerFootLeftActivity extends Activity implements OnClickListener {
	private String TAG = "RulerFootLeftActivity";
	...
	private List<FootRulerInfo> shoeSizeList;
	private FootRulerInfo info;
	private int[] sizeArr;
	private String realShoeSize;// 根据鞋码表得到的鞋码
	private String token = "";
	private static final int MSG_GET_RULERDATA = 0;
	private static final int MSG_POST_RULERDATA = 1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ruler_foot_left);
		mContext = this;
	...
	private void initView() {
		...
		findViewById(R.id.ruler_prev).setOnClickListener(this);
		tv_long = (TextView) findViewById(R.id.tv_shownum_l);
		tv_width = (TextView) findViewById(R.id.tv_shownum_w);
	...
	private void initData(FootRulerInfo footRulerInfo) {
		...
		if (!footRulerInfo.lftwidth.equals("")) {
			width = footRulerInfo.lftwidth;
		} else {
			width = "100";
		}
		if (!footRulerInfo.lftlong.equals("")) {
			height = footRulerInfo.lftlong;
		} else {
			height = "270";
		}
		tv_long.setText(height);
	...
	public void onClick(View v) {
		...
		case R.id.ruler_next:
			...
			SpfUtil.keepFtSize(mContext, lengthStr, Constants.LFTHEIGHT);
			SpfUtil.keepFtSize(mContext, widthStr, Constants.LFTWIDTH);
			updateRulerData();
			break;
		case R.id.ruler_prev:
			finish();
			startActivity(new Intent(this, RulerFootRightActivity.class));
			break;
		...
		public void onHttpPostSuccess(int type, int statusCode,
			...
			case MSG_POST_RULERDATA:
				...
				try {
					...
					// TODO 自动生成的 catch 块
					e.printStackTrace();
					Log.i(TAG, "MSG_POST_RULERDATA解析失败");
	...
	private void updateRulerData() {
		DialogUtil.showCancelProgressDialog(mContext, "", "请稍等", true, true);
		SimpleDateFormat sDateFormat = new SimpleDateFormat("yyyy-MM-dd");
		String date = sDateFormat.format(new java.util.Date());
		RequestParams params = new RequestParams();
		params.put("useid", SpfUtil.readUserId(mContext));
		params.put("calltype", ClientConstant.ADDRULERDATA);
		params.put("rftlong", SpfUtil.readFtSize(mContext, Constants.RFTHEIGHT));
		params.put("rftwidth", SpfUtil.readFtSize(mContext, Constants.RFTWIDTH));
		params.put("lftlong", SpfUtil.readFtSize(mContext, Constants.LFTHEIGHT));
		params.put("lftwidth", SpfUtil.readFtSize(mContext, Constants.LFTWIDTH));
		params.put("nickname", nickname);
		params.put("createtime", date);
		PostAsyncTask postTask = new PostAsyncTask(mHandler);
		postTask.startAsyncTask(mContext, callback, MSG_POST_RULERDATA, params,
				ClientConstant.GET_FOOTDATA_URL);
	}
	...
	protected void onResume() {
		super.onResume();
		MobclickAgent.onPageStart(ClassType.RulerFootLeftActivity);
		MobclickAgent.onResume(mContext);
		// height = SpfUtil.readFtSize(mContext, Constants.LFTHEIGHT);
		// width = SpfUtil.readFtSize(mContext, Constants.LFTWIDTH);
	...
	protected void onPause() {
		super.onPause();
		MobclickAgent.onPageEnd(ClassType.RulerFootLeftActivity);
